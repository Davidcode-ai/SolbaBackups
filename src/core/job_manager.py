...
class JobManager:
    ...
    async def run_job(
        self, job_id: int, db_session: Session, trigger: str = "manual"
    ) -> None:
        """
        ...
        run = crud.run_create(
            db_session, job_id=job.id, job_name=job.name, trigger=trigger
        )
        crud.log_add(
            db_session,
            run.id,
        ...
        try:
            ...
            if job.dest_retention_days and job.dest_retention_days > 0:
                crud.log_add(
                    ...
                    "INFO",
                    "cleanup",
                    f"Ejecutando política de retención ({job.dest_retention_days} días)...",
                )
            ...
            crud.run_finish(
                db_session, run.id, status="failed", error_message=error_msg
            )
